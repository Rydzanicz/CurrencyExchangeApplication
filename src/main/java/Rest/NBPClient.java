...
import Rest.Table_A_B.RateA_B;
import Rest.Table_AllCurrency.RateAllCurrency;
import Rest.Table_C.RateC;
...
public class NBPClient {
    ...
    public CurrencyRateList getCurrencyExchangeRateMedium(CurrencyRateList currencyRateList, LocalDate fromLocalDate, LocalDate toLocalDate) throws IOException {
        ...
        RateA_B rateA_b = new Gson().fromJson(reader, RateA_B.class);
        Collection<CurrencyRate> localCurrencyRate = rateA_b.getRates().stream().map(x -> new CurrencyRate(currencyRateList.getCurrency(),
                Optional.empty(), Optional.empty(), Optional.empty(), Optional.of(LocalDate.parse(x.getEffectiveDate())), Optional.of(x.getMid()))).collect(Collectors.toCollection(LinkedList::new));
